...
Tock met à disposition un framework complet d'internationalisation. Il est activé par défaut en mode _Bot API_
 (par exemple sur la [plateforme de démonstration](https://demotock-production-admin.vsct-prod.aws.vsct.fr/)).
...
Dans _Tock Studio_, la section _Build_ > _i18n_ permet de gérer les libellés des réponses du bot. Voir [_Le menu Build_](studio/build.md).
