...
def main():
    ...
    try:
        ...
        input_config = RunVectorisationInput.from_json_file(
        ...
        df = pd.read_csv(
            data_csv_file, delimiter='|', quotechar='"', header=0, dtype=str
        )
        ...
        for doc, (_, row) in zip(docs, df_filtered_clone.iterrows()):
            ...
            doc.metadata['id'] = str(
                uuid4()
            )  # An uuid for the doc (will be used by TOCK)
            # Add source metadata regardless of ignore_source
            doc.metadata['reference'] = row['source']

        logger.debug(
            f"Split texts in {input_config.chunk_size} characters-sized chunks"
        )
        ...
        if index_name is None:
            ...
            index_name = normalize_index_name(
                input_config.vector_store_setting.provider,
                input_config.bot.namespace,
                input_config.bot.bot_id,
                session_uuid,
            )
