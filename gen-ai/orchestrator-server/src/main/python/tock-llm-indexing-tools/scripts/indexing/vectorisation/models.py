...
class RunVectorisationInput(FromJsonMixin):
    ...
    embedding_max_chunks: Optional[int] = Field(
        description='Maximum number of chunks to index (useful for testing). '
                    'All chunks are indexed when null.',
        default=None,
        ge=1,
    )
    ...
    def format(self):
        ...
        details_str = f"""
                ...
                Max chunks to index     : {self.embedding_max_chunks or 'unlimited'}
