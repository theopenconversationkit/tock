...
def get_sessions(_dataset_id):
    ...
    logging.info(
        f'Call the LangSmith API to get run sessions for dataset_id={_dataset_id}.'
    )
...
def get_dataset_examples(nb_example):
    ...
    while counter > 0:
        logging.info(
            f'Call the LangSmith API to get {limit} dataset examples, starting from {offset}.'
        )
        ...
        if dataset_examples_response.status_code == 200:
            ...
            logging.debug(
                f'Number of examples obtained = {len(dataset_examples_content)}'
            )
            logging.debug(
                f'Number of examples collected = {len(examples)}/{nb_example}'
            )
        else:
            logging.error(
    ...
    logging.info(
        f'Fetched dataset examples = {len(examples)}/{nb_example} -> {len(examples) / nb_example * 100:.2f}%'
    )
    return examples
...
def get_example_runs_content(example_id):
    ...
    logging.info(
        f'Call the LangSmith API to get dataset runs for the example_id={example_id}.'
    )
    example_runs_response = requests.post(
        f'{base_url}/runs/query',
        json={
            'reference_example': [example_id],
            'is_root': 'true',
...
def append_example_runs(dataset_example, _session_ids):
    ...
    for _id in _session_ids:
        run = first_true(
            example_runs_content['runs'], predicate=lambda x: x['session_id'] == _id
        )
        if run is None:
            csv_line.append('')
            csv_line.append('')
            ...
            csv_line.append(
                ','.join(
                    [
                        doc['metadata']['url']
                        for doc in run['outputs']['source_documents']
                    ]
                )
            )
...
if __name__ == '__main__':
    ...
    try:
        logging.info(
            f'Call the LangSmith API to get the dataset information for dataset_id={dataset_id}.'
        )
        ...
        if dataset_info_response.status_code != 200:
            logging.error(
                f'Failed to get dataset information. \n'
        ...
        for example in dataset_examples:
            ...
            logging.info(
                f'Example processed : {index}/{example_counter} - Progression : {progress:.2f}%'
            )
            index += 1
    ...
    logging.info(
        'End of execution. (Duration : %.2f seconds)', time.time() - start_time
    )
