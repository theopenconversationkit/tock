...
def create_excel_output(iterations: list[str], dataset_items, output_file):
    ...
    wb = load_workbook("examples/generate_dataset_input.example.xlsx")
    sheet = wb.active

    for i in range(len(iterations)):
        start_row = 6 + 6 * i
        # Merge cells (example 6B:11B)
        sheet.merge_cells(start_row=start_row, start_column=2, end_row=11+6*i, end_column=2)
    ...
    for i in range(len(dataset_items)):
        ...
        sheet[f"{col_letter}2"] = dataset_items[i][0] # Topic
        sheet[f"{col_letter}3"] = dataset_items[i][1] # Question
        sheet[f"{col_letter}4"] = dataset_items[i][2] # Expected response
        counter = 2
