...
def _extract_dataset_items(filename: str, locale: str, no_answer: str) -> list[DatasetItem]:
    dataset_items = []
    input_df = pd.read_excel(filename, header=None)
    # Replace NaN with None in the DataFrame
    df = input_df.map(lambda x: None if pd.isna(x) else x)
    # Data extraction
    topics = df.iloc[1, 8:].tolist()     # Line 1: Topic
    questions = df.iloc[2, 8:].tolist()  # Line 2: Question
    answers = df.iloc[3, 8:].tolist()    # Line 3: Expected answer
    # DatasetItems
