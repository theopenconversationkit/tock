...
from gen_ai_orchestrator.routers.requests.requests import DocumentCompressorProviderSettingStatusQuery
...
async def get_document_compressor_provider_by_id(
    ...
    # Query validation
    validate_document_compressor_provider(request, provider_id)
...
async def get_document_compressor_provider_setting_by_id(
    request: Request, provider_id: DocumentCompressorProvider
...
async def check_document_compressor_provider_setting(
    request: Request, provider_id: str, query: DocumentCompressorProviderSettingStatusQuery
    ...
    Args:
        request: The http request
        provider_id: The provider id
        query: The query of the Document Compressor Provider Setting to be checked
    ...
    try:
        ...
        check_document_compressor_setting(query.setting)
...
def validate_query(request: Request, provider_id: str, setting: DocumentCompressorSetting):
    """
    ...
    logger.debug('Document Compressor setting - Query validation')
    validate_document_compressor_provider(request, provider_id)
...
def validate_document_compressor_provider(request: Request, provider_id: str):
    """
    ...
    if not DocumentCompressorProvider.has_value(provider_id):
        raise GenAIUnknownDocumentCompressorProviderException(
            create_error_info_not_found(
                request, provider_id, [provider.value for provider in DocumentCompressorProvider]
            )
        )
