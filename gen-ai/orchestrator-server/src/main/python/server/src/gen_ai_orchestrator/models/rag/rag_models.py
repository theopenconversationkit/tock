...
class ChunkInfos(BaseModel):
    ...
    used_in_response: Optional[bool] = Field(
        description='It indicates whether the chunk is actually used to generate the response.',
        examples=[True],
        default=False,
    )
...
class LLMCondensedQuestion(BaseModel):
    condensed_question: str = Field(
        description='The condensed question generated by the LLM.'
    )
    key_words: Optional[List[str]] = Field(
        description='A list of key words.',
        default=None,
    )
...
class LLMAnswer(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        description="Confidence score assigned to the answer provided to the user's question",
        examples=[0.93],
        default=None,
    )
    display_answer: Optional[bool] = Field(
        description='It indicates if we should display answer or not.',
        examples=[True],
        default=False,
    )
    ...
    understanding: Optional[str] = Field(
        description='LLM understanding of user query.',
        default=None,
    )
    redirection_intent: Optional[str] = Field(
        description='The intent to use to redirect the conversation flow.', default=None
    )
    context_usage: Optional[List[ChunkInfos]] = Field(
...
class RAGDebugData(QADebugData):
    ...
    condensing_llm_answer: LLMCondensedQuestion = Field(
        description='The LLMs condensed answer.'
    )
