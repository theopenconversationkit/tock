...
Answer in {locale}:""",
        ...
        "document_search_params": {
            "provider": "OpenSearch",
            "filter": [{"term": {"metadata.index_session_id.keyword": "352d2466-17c5-4250-ab20-d7c823daf035"}}],
            "k": 4,
        },
    ...
    mocked_get_callback_handler_factory.assert_called_once_with(setting=request.observability_setting)
    ...
    mocked_get_document_compressor_factory(setting=request.compressor_setting)
    ...
    mocked_rag_guard.assert_called_once_with(inputs, llm_answer, response, request.documents_required)
...
def test_guardrail_parse_succeed_with_toxicities_encountered(
    ...
    guardrail = get_guardrail_factory(
        BloomzGuardrailSetting(provider="BloomzGuardrail", max_score=0.5, api_base="http://test-guard.com")
    ).get_parser()
    rag_response = {
        "answer": {
            "status": "",
