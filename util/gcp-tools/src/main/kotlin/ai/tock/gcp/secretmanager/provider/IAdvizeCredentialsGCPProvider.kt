...
/**
 * IAdvize credentials GCP provider
 */
class IAdvizeCredentialsGCPProvider: CredentialsProvider {

    private val secretDAO: SecretDAO get() = injector.provide()

    override val type: CredentialsProviderType
        get() = CredentialsProviderType.GCP_SECRET_MANAGER

    /**
     * Return iAdvize credentials, from an GCP Secret Manager vault.
     */
    override fun getCredentials(): Credentials {
        return Json.decodeFromString(
            secretDAO.getSecret(EnvConfig.gcpIAdvizeCredentialsSecretId)
        )
    }
}