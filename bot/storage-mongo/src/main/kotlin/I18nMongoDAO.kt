...
internal object I18nMongoDAO : I18nDAO {
    ...
    private val col =
        database.getCollection<I18nLabel>().apply {
            ensureIndex(Namespace, Category)
        }
    private val asyncCol = asyncDatabase.getCollection<I18nLabel>()
    private val alternativeIndexCol =
        database.getCollection<I18nAlternativeIndex>().apply {
    ...
    private val statCol =
        database.getCollection<I18nLabelStat>().apply {
    ...
    private fun sortLabels(list: List<I18nLabel>): List<I18nLabel> = list.sortedWith(compareBy({ it.category }, { it.findLabel(defaultLocale, null)?.label ?: "" }))
    ...
    private fun sortLocalizedLabels(label: I18nLabelContract): I18nLabelContract = label.withUpdatedI18n(sortLocalizedLabels(label.i18n), version = label.version?.apply(Int::inc))
    ...
    override fun getLabels(
        namespace: String,
        filter: I18nLabelFilter?,
    ): List<I18nLabel> {
        val labelIds: Set<Id<I18nLabel>>? =
            filter?.notUsedSince?.let { notUsedSinceDays ->
    ...
    private fun I18nLabelFilter.toFilterList(labelIds: Set<Id<I18nLabel>>?): Iterable<Bson?> =
        listOfNotNull(
            label?.let {
                or(
    ...
    override fun deleteByNamespaceAndId(
        namespace: String,
        id: Id<I18nLabel>,
    ) {
    ...
    private fun alternativeIndexesFilter(
        label: I18nLabel,
        localized: I18nLocalizedLabel,
        contextId: String,
    ): Bson {
