...
class I18nTranslatorTest {
    ...
    @Test
    fun `translate with list args call translate with varargs`() {
        val impl = spyk(Impl())

        every { impl.translate("text", "a", "b") } returns "ok".raw

        val result = impl.translate("text", listOf("a", "b"))

        assertEquals("ok".raw, result)
    }
}
