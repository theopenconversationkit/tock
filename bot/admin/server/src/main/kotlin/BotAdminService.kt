...
object BotAdminService {
    ...
    /**
     * Handle the current new story and remove rag story definition if new story uses unknown as an intent
     */
    private fun handleCurrentRagStory(story: BotStoryDefinitionConfiguration) {
        if (story.intent.name == RagStoryDefinition.OVERRIDDEN_UNKNOWN_INTENT) {
            //check if story backup is already present
            storyDefinitionDAO.getStoryDefinitionByNamespaceAndBotIdAndIntent(story.namespace,story.botId,RagService.unknownStoryBackupIntentName)?.let {
                badRequest("Story ${it.name} cannot be created since the is an existing unknown backup and Rag is active")
            }

            //otherwise create it and delete RagStory
            storyDefinitionDAO.getRagStoryDefinitionByNamespaceAndBotId(story.namespace, story.botId)?.let {
                deleteStory(story.namespace, it._id.toString())
            }
            //deactivate RAG config if a new story is created with unknown intent
            ragConfigurationDAO.findByNamespaceAndBotId(story.namespace, story.botId)?.let { currentRagConfig ->
