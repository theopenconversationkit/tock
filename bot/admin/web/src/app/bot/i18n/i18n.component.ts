...
export class I18nComponent extends I18nController implements OnInit {
  ...
  complete() {
    ...
    this.botService.completeI18nLabels(this.i18n).subscribe((r) => {
      ...
      if (n === 0) {
        this.toastrService.show(`No label translated`, 'UPDATE', {
          duration: 2000
        });
      } else if (n === 1) {
        this.toastrService.show(`1 label translated`, 'UPDATE', {
          duration: 2000
        });
      } else {
        this.toastrService.show(`${n} labels translated`, 'UPDATE', {
          duration: 2000
        });
      }
    });
  ...
  validateAll() {
    ...
    this.botService.saveI18nLabels(this.i18n).subscribe((_) =>
      this.toastrService.show(`All labels validated`, 'Validate', {
        duration: 3000
      })
    );
  }
  ...
  downloadCsv(all: boolean) {
    if (all) {
      this.botService.downloadAllI18nLabelsCsv().subscribe((blob) => {
        ...
        this.toastrService.show(`Export provided`, 'Export', {
          duration: 2000
        });
      });
    } else {
