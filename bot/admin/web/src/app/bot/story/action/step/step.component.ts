...
export class StepComponent implements OnInit {
  ...
  checkStep() {
    if (this.step.new) {
      ...
      if (invalidMessage) {
        this.dialog.notify(this.transloco.translate('bot.story-action-step.errorMessage', { message: invalidMessage }));
        return;
  ...
  getMetricLabel(metric: StoryStepMetric): string {
    ...
    return this.transloco.translate('bot.story-action-step.deletedIndicatorValue', {
      indicatorName: metric.indicatorName,
      indicatorValueName: metric.indicatorValueName
    });
  }
