...
export class UsersComponent extends ScrollComponent<UserReport> {
  ...
  addDialogToTestPlan(planId: string, dialog: DialogReport) {
    if (!planId) {
      this.toastrService.show(`Please select a Plan first`, 'Error', {
        duration: 3000
      });
      return;
    }
    this.analytics.addDialogToTestPlan(planId, dialog.id).subscribe((_) =>
      this.toastrService.show(`Dialog added to plan`, 'Dialog Added', {
        duration: 3000
      })
    );
  }
