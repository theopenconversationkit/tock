...
export function parseImportFile(raw: string, locale?: string | null): ParsedImportFile {
  ...
  if (isKnowledgeBaseEnvelope(parsed)) {
    if (parsed.version !== KNOWLEDGE_BASE_EXPORT_VERSION) throw new KnowledgeBaseImportFormatError('knowledge-base.import.error_version');
...
function isFaqExport(value: unknown): boolean {
  ...
  return value.every(
    (row) =>
      !!row && typeof row === 'object' && Array.isArray((row as RawFaqEntry).utterances) && Array.isArray((row as RawFaqEntry).answer?.i18n)
  );
}
...
function mapKnowledgeBaseEnvelope(envelope: KnowledgeBaseExportEnvelope): ParsedImportRow[] {
  return envelope.entries.map((entry) => {
    ...
    return {
      payload: {
        title,
        searchHints: stringList(entry.searchHints),
        content,
...
function mapFaqEntry(row: RawFaqEntry, locale: string): ParsedImportRow {
  ...
  const utterances = stringList(row.utterances);
  ...
  const footnotes = Array.isArray(row.footnotes) ? row.footnotes : [];
  const sourceUrl = httpUrlOrNull(footnotes[0]?.url);
  if (footnotes.length && !sourceUrl) issues.push('knowledge-base.import.issue_footnote_without_url');
  ...
  return {
    ...
    sourceId: stringValue(row._id) || stringValue(row.id) || null,
    issues,
    rejected: !title || !content,
...
function pickLocalizedLabel(row: RawFaqEntry, locale: string): string | null {
  const labels = (row.answer?.i18n ?? []).filter((label) => label?.locale === locale && !!stringValue(label.label));
  if (!labels.length) return null;

  const generic =
    labels.find((label) => !label.connectorId && (label.interfaceType === 'textChat' || label.interfaceType === 0)) ??
    labels.find((label) => !label.connectorId);
  return stringValue((generic ?? labels[0]).label);
}
...
function collectLocales(rows: RawFaqEntry[]): string[] {
  ...
  rows.forEach((row) => {
    (row.answer?.i18n ?? []).forEach((label) => {
      if (typeof label?.locale === 'string' && stringValue(label.label)) locales.add(label.locale);
    });
    if (typeof row.language === 'string' && row.language) locales.add(row.language);
  });
