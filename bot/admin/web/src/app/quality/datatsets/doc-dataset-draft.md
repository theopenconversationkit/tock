...
- il faut filtrer les secrets avant persistance, en particulier les `apiKey`,
- pour exécuter `/test/talk`, le worker doit aussi résoudre la `locale` et le `botApplicationConfigurationId` au moment du lancement ; ces informations peuvent rester internes au run si on ne veut pas les exposer au front
...
3. on résout la config REST de test et la locale courante,
4. on snapshote les settings RAG courants,
5. on crée un `DatasetRun` en `QUEUED`,
6. on précrée les `DatasetRunQuestionResult` en `PENDING`,
7. le worker récupère le run,
8. il passe le run en `RUNNING`,
9. il exécute les questions une par une,
10. il met à jour la progression après chaque question,
11. à la fin il passe le run en `COMPLETED` ou `CANCELLED`.
