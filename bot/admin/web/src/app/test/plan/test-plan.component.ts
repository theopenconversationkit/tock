...
export class TestPlanComponent implements OnInit {
  ...
  executeXRay() {
    if (this.xray.testPlanKey.trim().length === 0) {
      this.toastrService.show(`Please specify a plan key`, 'Error', {
        duration: 2000
      });
    } else {
  ...
  createTestPlan() {
    if (!this.testPlanName || this.testPlanName.trim().length === 0) {
      this.toastrService.show(`Please enter a valid name`, 'Error', {
        duration: 5000
      });
      return;
    }
    ...
    this.test
      ...
      .subscribe((_) => {
        ...
        this.reload();
        this.toastrService.show(`New test plan saved`, 'New Test Plan', {
          duration: 2000
        });
      });
  }
  ...
  deleteTestPlan(plan: TestPlan) {
    this.test.removeTestPlan(plan._id).subscribe((_) => {
      this.reload();
      this.toastrService.show(`Plan ${plan.name} deleted`, 'Delete', {
        duration: 2000
      });
    });
  }
