...
class MattermostConnector(
    ...
    private val client: MattermostClient = MattermostClient(url)
    ...
    override fun register(controller: ConnectorController) {
        controller.registerServices(path) { router ->
            ...
            router.post(path).handler { context ->
                logger.info { "Handle mattermost request ${context.body().asString()}" }
                ...
                try {
                    val body = when(context.request().getHeader("Content-Type")) {
                        "application/x-www-form-urlencoded" -> context.convertUrlEncodedStringToJson()
    ...
    private fun sendMessage(message: String, delayInMs: Long) {
        executor.executeBlocking(Duration.ofMillis(delayInMs)) {
            ...
            client.sendMessage(token, mattermostMessage)
        }
    }
}
