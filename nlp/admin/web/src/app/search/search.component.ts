    ...
    if (this.selectedSentences && this.selectedSentences.length === 0) {
      ...
      this.nlp
        ...
        .subscribe((r) => {
          const n = r.nbUpdates;
          if (n === 0) {
            this.toastrService.show(`No sentence updated`, 'UPDATE', {
              duration: 2000
            });
          } else if (n === 1) {
            this.toastrService.show(`1 sentence updated`, 'UPDATE', {
              duration: 2000
            });
          } else {
            this.toastrService.show(`${n} sentences updated`, 'UPDATE', {
              duration: 2000
            });
          }
          this.scroll.refresh();
    ...
    if (!this.targetLocale || this.targetLocale.length === 0) {
      ...
      this.nlp
        ...
        .subscribe((r) => {
          ...
          if (n === 0) {
            this.toastrService.show(`No sentence translated`, 'UPDATE', {
              duration: 2000
            });
          } else if (n === 1) {
            this.toastrService.show(`1 sentence translated`, 'UPDATE', {
              duration: 2000
            });
          } else {
            this.toastrService.show(`${n} sentences translated`, 'UPDATE', {
              duration: 2000
            });
          }
        });
